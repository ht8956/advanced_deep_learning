...
class LoRALinear(HalfLinear):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #raise NotImplementedError()
        x_cast = x.to(torch.float16)
        base_output = torch.nn.functional.linear(x_cast, self.weight, self.bias)
        lora_output = self.lora_b(self.lora_a(x.float()))
