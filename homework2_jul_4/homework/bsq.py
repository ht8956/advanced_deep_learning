...
class BSQPatchAutoEncoder(PatchAutoEncoder, Tokenizer):
    ...
    def encode_index(self, x: torch.Tensor) -> torch.Tensor:
        ...
        z = super().encode(x)
        idx = self.bsq.encode_index(z)
        return idx if has_batch_dim else idx[0]
    ...
    def decode_index(self, x: torch.Tensor) -> torch.Tensor:
        ...
        img = super().decode(z)
        return img if has_batch_dim else img[0]
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        x_hat = super().decode(self.bsq.decode(z_q))

        cnt = torch.bincount(self.bsq._code_to_index(z_q).flatten(), minlength=2**self.codebook_bits)
        stats = {
            "cb0": (cnt == 0).float().mean().detach(),
            "cb2": (cnt <= 2).float().mean().detach(),
        }
