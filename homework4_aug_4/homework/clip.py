...
class CLIP(nn.Module):
    def __init__(
        ...
        vision_hidden_size = getattr(self.vision_encoder.config, "hidden_size", None) or getattr(
            self.vision_encoder.config, "embed_dim", None
        )
        text_hidden_size = getattr(self.text_encoder.config, "hidden_size", None) or getattr(
            self.text_encoder.config, "d_model", None
        )

        if vision_hidden_size is None or text_hidden_size is None:
            raise ValueError("Unable to infer encoder hidden sizes for CLIP projections")
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        def pool_encoder_output(encoder_output: Any, mask: torch.Tensor | None = None) -> torch.Tensor:
            pooled_output = getattr(encoder_output, "pooler_output", None)
            ...
            token_positions = mask.long().sum(dim=1).clamp(min=1) - 1
        ...
        vision_embedding = pool_encoder_output(vision_outputs)
        text_embedding = pool_encoder_output(text_outputs, attention_mask)

        vision_feature = nn.functional.normalize(self.vision_projection(vision_embedding), dim=-1)
        text_feature = nn.functional.normalize(self.text_projection(text_embedding), dim=-1)
        logits_per_image = self.logit_scale.exp() * torch.matmul(vision_feature, text_feature.T)
...
def compute_clip_loss(
    ...
    if logits_per_image.shape[0] != logits_per_image.shape[1]:
        raise ValueError("CLIP training expects the same number of images and captions in each batch")
    ...
    text_loss = nn.functional.cross_entropy(logits_per_image.T, targets)
